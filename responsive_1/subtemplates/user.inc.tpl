...
{foreach from=$userdata item=row}
  <tr>
   ...
   <td data-header="{#user_hp#}"><span class="value">{if $row.user_hp!=''}<a href="{$row.user_hp}" title="{$row.user_hp}"><img src="{$THEMES_DIR}/{$theme}/images/homepage.png" alt="{#homepage#}" width="16" height="16" class="icon" /></a>{else}&nbsp;{/if}</span></td>
   <td data-header="{#user_email#}"><span class="value">{if $row.user_email}<a href="index.php?mode=contact&amp;user_id={$row.user_id}" title="{#mailto_user#|replace:"[user]":$row.user_name}"><img src="{$THEMES_DIR}/{$theme}/images/contact-email.svg" alt="{#email#}" width="16" height="16" class="icon" /></a>{else}&nbsp;{/if}</span></td>
