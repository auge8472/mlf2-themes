...
<li><a class="{if $data.$element.pid==0}opening-entry{else}following-entry{/if}{if $data.$element.new} new{/if}{if $data.$element.sticky==1} sticky{/if}{if $data.$element.locked==1} locked{/if}{if $data.$element.is_read} read{/if}" href="index.php?id={$data.$element.id}">{if $data.$element.spam==1}<span class="spam">{$data.$element.subject}</span>{else}{$data.$element.subject}{/if}</a>
...
{if $data.$element.category_name && $data.$element.pid==0 && $category<=0} <a href="index.php?mode=index&amp;category={$data.$element.category}" title="{#change_category_link#|replace:"[category]":$data.$element.category_name|escape:"html"}"><span class="category">{$data.$element.category_name}</span></a>{/if} · <span id="p{$data.$element.id}" class="tail">{if $data.$element.pid==0}<a href="index.php?mode=thread&amp;id={$data.$element.id}"><img src="{$THEMES_DIR}/{$theme}/images/thread-entries-nested.svg" alt="" width="12" height="12" /><span>{#open_whole_thread#}</span></a>{/if}{if $admin || $mod}
...
<a href="index.php?mode=posting&amp;delete_posting={$data.$element.id}&amp;csrf_token={$CSRF_TOKEN}&amp;back=index"><img src="{$THEMES_DIR}/{$theme}/images/marker-cross.svg" alt="" width="12" height="12" /><span>{#delete_posting_title#}</span></a>{/if}
{if $fold_threads==1 && $data.$element.pid==0 && $replies.$thread>0} <span class="replies" title="{*{if $replies.$thread==0}{#no_replies#}*}{if $replies.$thread==1}{#one_reply#}{else}{$smarty.config.several_replies|replace:"[replies]":$replies.$thread}{/if}">({$replies.$thread})</span>{/if}</span>
