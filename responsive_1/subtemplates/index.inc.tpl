...
{if $data.$element.category_name && $data.$element.pid==0 && $category<=0} <a href="index.php?mode=index&amp;category={$data.$element.category}" class="category" title="{#change_category_link#|replace:"[category]":$data.$element.category_name|escape:"html"}">{$data.$element.category_name}</a>{/if} · 
